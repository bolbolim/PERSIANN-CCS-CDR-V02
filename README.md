**PERSIANN-CCS-CDR Version 2.0**

This repository contains code and resources related to the PERSIANN-CCS-CDR V2.0 dataset, a global high-resolution precipitation climate record.

**Citation**

If you find this work or the dataset useful for your research, please cite the following paper:

https://doi.org/10.1038/s41597-026-06625-5


    Bolboli Zadeh, M., Nguyen, P., Hsu, KL. et al. A Global High-Resolution Precipitation Climate Record: PERSIANN-CCS-CDR Version 2.0. Sci Data 13, 314 (2026). https://doi.org/10.1038/s41597-026-06625-5

BibTeX:

    @article{bolbolizadeh2026PERSIANN,
        title={A Global High-Resolution Precipitation Climate Record: PERSIANN-CCS-CDR Version 2.0},
        author={Zadeh, Mohammad Bolboli and Nguyen, Phu and Hsu, Kuo-Lin and AghaKouchak, Amir and Ung, Tu Thanh and Sorooshian, Soroosh},
        journal={Scientific Data},
        volume={13},
        number={314},
        year={2026},
        publisher={Nature Publishing Group},
        doi={10.1038/s41597-026-06625-5},
        url={https://doi.org/10.1038/s41597-026-06625-5}
    }

**Dataset Access**

The full PERSIANN-CCS-CDR-V02 dataset is available for download via Dryad:

Download Link: 
https://doi.org/10.5061/dryad.n8pk0p38q
